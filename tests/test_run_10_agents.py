...
class TestParamVariants:
    def test_has_16_variants(self):
        # 12 upstream navigation variants + 4 fork battle-strategy variants
...
class TestScore:
    # NOTE: score() lives in evolve.py and is exhaustively tested in test_evolve.py
    # (the fork rewrote the scoring formula). Here we only assert the re-export is
    # wired up and behaves monotonically — exact constants live with test_evolve.
...
class TestHasPaper:
    ...
    def test_false_when_not_installed(self):
        with patch("run_10_agents.subprocess.run", side_effect=FileNotFoundError("no paper")):
            assert _has_paper() is False

    def test_false_on_timeout(self):
        with patch("run_10_agents.subprocess.run", side_effect=sp.TimeoutExpired("paper", 5)):
            assert _has_paper() is False
...
class TestRunOneAgent:
    ...
    def test_success(self):
        ...
        with patch("run_10_agents.subprocess.Popen", return_value=_fake_proc(stdout=json.dumps(fitness))):
    ...
    def test_timeout_kills_tree_and_returns_error(self):
        ...
        with patch("run_10_agents.subprocess.Popen", return_value=proc), patch("run_10_agents._terminate_tree") as term:
    ...
    def test_file_not_found_returns_error_without_terminate(self):
        ...
        with (
            patch("run_10_agents.subprocess.Popen", side_effect=FileNotFoundError("no claude")),
    ...
    def test_invalid_json_returns_error(self):
        ...
        with patch("run_10_agents.subprocess.Popen", return_value=_fake_proc(stdout="not json at all")):
    ...
    def test_params_embedded_in_prompt(self):
        ...
        assert "label" not in json.loads(prompt.split("EVOLVE_PARAMS='")[1].split("'")[0])
    ...
    def test_use_paper_builds_paper_cmd_and_strips_keys(self):
        ...
        with (
            ...
            patch.dict(os.environ, {"ANTHROPIC_API_KEY": "secret", "ANTHROPIC_BASE_URL": "http://x"}),
        ):
...
class TestTerminateTree:
    def test_kills_group_then_reaps(self):
        ...
        with patch("run_10_agents.os.getpgid", return_value=4321), patch("run_10_agents.os.killpg") as killpg:
    ...
    def test_swallows_wait_timeout(self):
        ...
        with patch("run_10_agents.os.getpgid", return_value=4321), patch("run_10_agents.os.killpg"):
            _terminate_tree(proc)  # must not raise
...
class TestMainGuard:
    def test_dunder_main_calls_main(self, tmp_path, capsys):
        ...
        fitness = {"final_map_id": 0, "badges": 0, "party_size": 1, "battles_won": 0, "stuck_count": 0, "turns": 0}
