...
from paper_reader import TapeEntry, TapeSession, TokenUsage, ToolResult, ToolUse
...
class TestObserveSession:
    ...
    def test_extracts_token_usage(self, tmp_path):
        ...
        session = _make_session(
            entries=[
                TapeEntry(
                    type="assistant", token_usage=TokenUsage(input_tokens=1000, output_tokens=200, cache_read=800)
                )
            ]
        )
        results = obs.observe_session(session)
    ...
    def test_write_tool_with_empty_summary_skipped(self, tmp_path):
        obs = _make_observer(tmp_path)
        session = _make_session(
            entries=[TapeEntry(type="assistant", tool_uses=[ToolUse(id="tu-1", name="Write", input_summary="")])]
        )
        results = obs.observe_session(session)
    ...
    def test_non_write_tools_not_tracked(self, tmp_path):
        obs = _make_observer(tmp_path)
        session = _make_session(
            entries=[TapeEntry(type="assistant", tool_uses=[ToolUse(id="tu-1", name="Read", input_summary="/some.py")])]
        )
        results = obs.observe_session(session)
        assert not any("File created" in o.content for o in results)
...
class TestWriteObservations:
    ...
    def test_no_duplicate_date_headers(self, tmp_path):
        ...
        obs.write_observations(
            [
                Observation(
                    referenced_time="2026-03-09T12:00:00Z", content="More stuff", source_session="sess1234-5678"
                ),
            ]
        )
...
class TestLoadState:
    ...
    def test_reads_existing_state(self, tmp_path):
        obs = _make_observer(tmp_path)
        obs.memory_dir.mkdir(parents=True, exist_ok=True)
        obs.state_path.write_text(json.dumps({"reader": "fake-v1", "processed_sessions": ["a", "b"]}))
    ...
    def test_resets_on_reader_mismatch(self, tmp_path, capsys):
        ...
        obs.state_path.write_text(json.dumps({"reader": "sqlite-sha", "processed_sessions": ["sha1", "sha2"]}))
